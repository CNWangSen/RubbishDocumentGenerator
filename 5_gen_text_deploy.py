...
class Gen:
	...
	def run(self):
		...
		for f in os.listdir("req/modules/"):
			...
			mods += modname+","
		if(self.override==False and os.path.exists("text/deploy/"+modname+".txt")):
			return
		Content = self.GetPromptFromDS("无人平台分布式协同与控制基础服务平台基于KubeEdge构建，它有八个模块分别是："+mods+"，给出无人平台分布式协同与控制基础服务平台的部署方案。写成多个自然段，不少于3000字！只回答汉字、逗号和句号，不要有数字或者引号等其他字符。不要有多余的回答！我会直接把你的回答填入投标文档的技术方案部分！")
		self.writeTxt("text/deploy/main.txt",Content)
